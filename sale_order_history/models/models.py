...
class SaleOrderHistory(models.TransientModel):
    ...
    def _onchange_order_line_id(self):
        if self.order_line_id:
            product_order_lines = self.env['sale.order.line'].search([('product_id','=',self.order_line_id.product_id.id),('order_partner_id','=',self.order_line_id.order_partner_id.id)])
